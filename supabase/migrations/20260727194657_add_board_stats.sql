-- StillPoor public Board statistics.
--
-- Current Wealth includes confirmed Block sales only.
-- Active reservations temporarily occupy pricing positions.
-- Expired reservations are treated as available without modifying data.
    ...
    select
      ...
      case
        when
          current_pricing.current_price_sats
            is null
          or current_pricing.current_price_sats
            >= 500000
        then null
        else
          current_pricing.current_price_sats
  ...
  select
    ...
    case
      ...
      then null
        ...
        pricing_summary.blocks_until_price_increase
    end as blocks_until_price_increase,

    board_summary.next_available_block_number,

    (
      board_summary.claimed_blocks = 4096
    ) as sold_out
  from board_summary
